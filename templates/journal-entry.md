...
> 状态：<进行中 / 部分完结 / 已完结>
> Use when：开工新批次即由 new-batch.sh 实例化本模板（无需手工创建）。
> 来源：<用户反馈 / 用户裁决 / E2E / 顺带发现 / 审计>
...
<按时间与轮次追加（取证链锚点）：>
- 轮次编号：批内多轮修复用「N 轮」分节，每轮 = 反馈 → 取证 → 根因 → 修复 → 复验闭环
- 判定词：GREEN / RED（如实记录，RED 不是耻辱是证据）
- 用户裁决：「用户裁决：<原话/要点>（<日期>）」——同域多裁决以最新为准
- 证据对：before/after 成对落盘（基线 vs 修复后），标注 commit 与路径
- 遗留（如实）：未解决项与影响，不粉饰
