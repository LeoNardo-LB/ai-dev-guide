...
| 5 | P0-P4 五节标题各恰好一次且按序 | 门禁 9 强制 |
| 6 | 登记簿 ≤250 行——超了说明有完结内容未迁移 | 门禁 9 警告（提示迁移，不阻断） |
| 7 | 卡片（顶层 `- [` 行）只许出现在 P0-P4 节内，不放表头区 | 门禁 9 强制 |
| 8 | 账本变更（登记/注记/状态/迁移/journal 追加）经 scripts/backlog.sh 操作，卡片区与 journal 禁手工直编 | 纪律（脚本内置防呆：自动编号/计数器/迁入节格式） |
...
优先级 P0-P4 定义与 [ ]/[~]/[x] 状态流转的**唯一归宿**：[requirements.md](../workflows/requirements.md) §3/§4（CANON 标记锁定）。P4 = 外部前提阻塞：卡内必含「前提」行（前提是什么、为何现在做不了、满足后如何接管）。
