...
## 机械不变量（check.sh 门禁 9 强制）

| # | 不变量 |
|---|--------|
| 1 | 卡片 ≤3 行摘要 + 链接；需求全文/实现要点/验证证据一律写在链接目标 |
| 2 | 编号全局递增永不回收；头部维护「下一编号：**#N**」，N 大于全库（backlog+journal+specs）最大编号 |
| 3 | 顶层完结 [x] 残留为零——验收通过后当场迁入 journal |
| 4 | 本地链接全部存在；不引用 docs/archive/（归档引用只允许在 journal/spec） |
| 5 | P0-P3 四节标题各恰好一次且按序 |
| 6 | 登记簿 ≤250 行——超了说明有完结内容未迁移 |
