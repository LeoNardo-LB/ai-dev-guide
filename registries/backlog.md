...
**卡片格式**：标题（含全局编号）+ Tag + 状态 checkbox + ≤3 行摘要 + 链接。需求全文、实现要点、验证证据一律写在链接目标（spec/journal）中，不内联。**账本操作一律经 scripts/backlog.sh**（add/note/status/migrate/journal append；卡片区与 journal 禁手工直编——防覆写丢章与同号双卡）；登记新批次用 new-batch.sh；改动后跑 check.sh --deployed 校验机械不变量。

**编号**：全局递增，不回收。下一编号：**#2**。

**优先级定义**（P0-P4 唯一归宿：系统 workflows/requirements.md「优先级定义」节）：
...
| P3 | 观察项 / 低价值改进 |
| P4 | 外部前提阻塞：方向明确但前提在项目之外（上游/服务器/用户门槛），卡内必含「前提」行（前提是什么、为何现在做不了、满足后如何接管） |
