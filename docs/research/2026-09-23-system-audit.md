...
## 11. 复审（2026-09-23，v2.6.0 后系统性 re-check）

> 用户三问：状态机完整生命周期？符合自身定位？高内聚低耦合？——两路子代理独立复审 + 主代理终检 + 全部新发现当场修复（十门禁/selftest R1-R12 复验全绿）。

### 11.1 状态机生命周期终检（13 个 × 五要素：进入/退出/触发者/守卫/异常路径）

| 状态机 | 判定 | 说明 |
|--------|------|------|
| 卡片三态 | ✅ 闭环 | 守卫活+[X]拦截+[ ]→[x] 直迁已声明（复审补）+周期审查删卡授权 |
| 优先级 P0-P4 | ✅ 闭环 | prio 变迁+P4 复查出口（v2.6.0） |
| backlog.sh | ✅ 闭环 | 四修+R12 回归；复审补：python 内核下沉 _backlog_core.py（可单测）、ARGV 死代码清除 |
| journal | ✅ 纪律闭环 | append-only 豁免声明（复审补）；误迁撤销=git revert（在册遗留） |
| 版本阶梯 | ✅ 如实闭环 | 单轨注记+tag v2.6.0+人工核对项声明 |
| 文档退役 | ✅ 闭环 | 门禁 1/5 拦断链 |
| 验收 item | ✅ 闭环 | 终态含 SKIP+待补卡承接+BLOCKED 解除规则（复审补） |
| 债务 | ✅ 闭环 | 复审补：处理中→待处理（中止）、已接受→待处理（触发重开）回边 |
| ADR | ✅ 闭环 | 复审补：已接受→已废弃 边（原图表矛盾消除） |
| 批次 | 🟡 P2 | 关闭无机制（状态行手改，在册） |
| 裁决 | 🟡 P2 | 已迁卡 note 回写断裂（在册） |
| Spec | 🟡 P3 | 否决终态/批准权未定义（在册） |
| 文档生命周期 | 🟡 P3 | replaces 溯源无强制（在册） |

**9/13 五要素完整闭环，4 个附属机制 P2/P3 在册**（核心链路全部闭环）。

### 11.2 定位言行一致（README 六原则 × 实测）

指针不是百科 ✅（92/200 行）｜manifest 单源 ✅（幂等）｜每条机械规则配门禁 🟡（十门禁+复审再增强四项；5 条已知盲区在册：卡长/Tag 数/Use-when 触发动词/迁入节标题/六符号增殖）｜栈无关 ✅（正文零残留；复审修 2 处指向 example 的指针）｜登记三分离 ✅｜自举 🟡（复审修补 3 处仪式走样：批次 H1 格式/迁入节标准标题/历史引用勘误）。CHANGELOG 2.6.0 八项抽验全部属实（子代理沙盒复现）。

### 11.3 高内聚低耦合（主代理耦合图 + 子代理纯度评审）

- **耦合**：26 对双向引用经抽验证实均为「归宿指针型」（verify↔acceptance 互指对方唯一归宿，无规则复制）——文档系统低耦合的正确形态；workflows 域内互指 32 vs 域外引用 33（L1 流程引 L2 标准属设计分层）；部署面→源仓面越界 1 处（onboarding→system-design，部署时被 scrub 清洗兜底）。
- **内聚**：code-style §6 显式分工条款为范本；**requirements.md 过载（≥5 变化理由）**——P2 拆薄建议在册（§6/§7 外移+裁决外移，降为 3 个变化理由）；backlog.sh 双语言分裂已修（下沉 _backlog_core.py）；判定次序/仪器判据/四类枚举存在 3-4 处近逐字复述——P2 收敛建议在册（各定正本+指针）。
- 门禁自身内聚：复审修门禁 8 双语言扫描范围不一致（.md.template 逃逸）+豁免清单单一正本标注。

### 11.4 修复批次自身的回归（两路复审交叉）

修复批次新引入缺陷 4 处，已全修并固化防复发：①双 §3.3（门禁 6 新增小节编号唯一检查，双编号检出验证 ✓）②registries/backlog.md 状态口径漏同步 ③requirements SKIP 行三列表格丢列（命题丢失）④**gate_backlog 外部 #编号误报**（P4 前提行写「上游 #789」即瘫痪账本——max_id 收窄为卡片标题行，两用例复验 ✓）。「改 A 忘 B」模式与本审计 C-P1-1 教训同源——编号唯一检查已固化，连锁 grep 纪律入编辑卡补律。
