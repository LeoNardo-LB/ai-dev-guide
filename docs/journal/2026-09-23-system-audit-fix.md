# 2026-09-23 <批次：system-audit-fix>
...
2026-09-23 复审记录（v2.6.0 后 re-check）：两路子代理+主代理终检。发现并修复修复批次新引入缺陷 4 处（双 §3.3 编号/registries 状态口径漏同步/SKIP 行三列丢列/gate_backlog 外部 #编号误报——P4 前提行含上游单号即瘫痪账本，max_id 已收窄）；生命周期五补（[ ]→[x] 直迁声明/append-only 豁免/BLOCKED 解除/债务中止与重开回边/ADR 废弃边）；_backlog_core.py 下沉（可单测）；门禁再增强（小节编号唯一/第 N 节文本引用 31 处纳入/子节精确校验/门禁 8 纳入 template）。GREEN：十门禁+selftest R1-R12 全绿，双编号检出与外部编号用例均验证。详见审计报告第 11 节。
