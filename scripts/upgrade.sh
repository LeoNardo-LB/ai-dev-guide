...
# 只报告，不自动覆盖。
# 信任模型：基线是 init.sh 部署产物，手改基线可掩盖本地修改——勿手改（沙盒审计 O1 定规）。
# ============================================================
set -uo pipefail
...
if [ ! -d "$TGT/$SYS" ] || [ ! -f "$TGT/$SYS/scripts/check.sh" ]; then
  echo "✗ $TGT/$SYS 不存在或不是本系统部署目录（缺 scripts/check.sh）——第三参应传部署时 --dir 之名，默认 ai-dev-guide"
...
up_new, local_mod, missing, local_extra, clean, upstream_gone = [], [], [], [], [], []
for d in data['docs']:
    if d['plane'] != 'deployed':
        continue
    ...
    if hs and ht:
        ...
        # 基线记录过 = 部署时存在而上游已删除——与「本地新增」语义相反（沙盒审计 D2 定规）
...
print(f'比对模式: {mode}')
print(f'一致 {len(clean)} · 可直接覆盖 {len(up_new)} · 本地化修改 {len(local_mod)} · 上游有/部署缺 {len(missing)} · 上游已删除 {len(upstream_gone)} · 本地新增 {len(local_extra)}')
print()
